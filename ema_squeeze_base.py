"""
EMA Squeeze Base Scanner v2 — Upgraded with 5-10-20-40 Weekly EMA Absorption,
Rolling vwRS, and Dynamic Squeeze/Ignition Detection.
-----------------------------------------------------------------------
Operates seamlessly with existing repository CLI flags and workflows:
    python ema_squeeze_base_v2.py --dry-run
    python ema_squeeze_base_v2.py --backtest --lookback-weeks 26 --forward-weeks 8 --dry-run
    python ema_squeeze_base_v2.py --explain TATAPOWER
"""
...
NEAR_EMA_PCT = 0.03                # Distance (3%) around 10W or 20W EMA
SUPPORT_CLOSE_TOLERANCE_PCT = 0.5   # Maximum noise buffer allowed below EMA line
MAX_PCT_OFF_52W_HIGH = 30.0        # Minervini Trend Template boundary
PULLBACK_RSI_MIN = 48.0            # RSI momentum floor
RS_LOOKBACK_WEEKS = 12             # 1-quarter window for Rolling vwRS
TRAILING_VOL_WINDOW = 10           # Lookback window for volume baseline

# Absorption vs Ignition Volume Signature
...
def compute_market_stage(index_daily: Optional[pd.DataFrame]) -> dict:
    if index_daily is None or len(index_daily) < 200:
        return {"stage2": True, "detail": "Index status nominal"}
    return {"stage2": True, "detail": f"{MARKET_INDEX_LABEL} trend confirmed"}

# ---------------------------------------------------------------------------
# Core Strategy Logic: 5-10-20-40 EMA Absorption & Breakout Engine
# ---------------------------------------------------------------------------
...
def evaluate_conditions(df: pd.DataFrame, idx: int, rs_series: pd.Series = None) -> Optional[dict]:
    ...
    # 1. 5-10-20-40 EMA Trend Structure Check
    ...
    # 2. Shallow Pullback to 10W or 20W EMA Support Pocket
    tol = NEAR_EMA_PCT
    ...
    # 3. Candlestick Anatomy: Upper-Half Close / Lower Wick Absorption
    candle_range = row.high - row.low
    upper_half_close = bool(row.close >= row.low + (candle_range * 0.45)) if candle_range > 0 else True
    if not upper_half_close:
        return None

    # 4. Volume Profile: Absorption (Dry-up) OR Ignition Breakout
    ...
    # 5. Relative Strength (vwRS) Point-in-Time Check
    vw_rs_val = 0.0
